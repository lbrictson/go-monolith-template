...
type Session struct {
	...
	// MfaCompleted holds the value of the "mfa_completed" field.
	MfaCompleted bool `json:"mfa_completed,omitempty"`
	// Edges holds the relations/edges for other nodes in the graph.
	// The values are being populated by the SessionQuery when eager-loading is set.
	Edges             SessionEdges `json:"edges"`
	user_user_session *uuid.UUID
...
type SessionEdges struct {
	...
	UserSession *User `json:"user_session,omitempty"`
...
func (e SessionEdges) UserSessionOrErr() (*User, error) {
...
func (*Session) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case session.ForeignKeys[0]: // user_user_session
	...
	for i := range columns {
		...
		case session.FieldMfaCompleted:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field mfa_completed", values[i])
			} else if value.Valid {
				s.MfaCompleted = value.Bool
			}
		case session.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field user_user_session", values[i])
			} else if value.Valid {
				s.user_user_session = new(uuid.UUID)
				*s.user_user_session = *value.S.(*uuid.UUID)
			}
		default:
...
// QueryUserSession queries the "user_session" edge of the Session entity.
func (s *Session) QueryUserSession() *UserQuery {
	return NewSessionClient(s.config).QueryUserSession(s)
}
...
func (s *Session) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("mfa_completed=")
	builder.WriteString(fmt.Sprintf("%v", s.MfaCompleted))
	builder.WriteByte(')')
