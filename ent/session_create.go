...
func (sc *SessionCreate) SetMfaCompleted(b bool) *SessionCreate {
	sc.mutation.SetMfaCompleted(b)
	return sc
}

// SetNillableMfaCompleted sets the "mfa_completed" field if the given value is not nil.
func (sc *SessionCreate) SetNillableMfaCompleted(b *bool) *SessionCreate {
	if b != nil {
		sc.SetMfaCompleted(*b)
	}
	return sc
}
...
func (sc *SessionCreate) SetUserSessionID(id uuid.UUID) *SessionCreate {
	sc.mutation.SetUserSessionID(id)
	return sc
}

// SetNillableUserSessionID sets the "user_session" edge to the User entity by ID if the given value is not nil.
func (sc *SessionCreate) SetNillableUserSessionID(id *uuid.UUID) *SessionCreate {
	if id != nil {
		sc = sc.SetUserSessionID(*id)
	}
	return sc
}

// SetUserSession sets the "user_session" edge to the User entity.
func (sc *SessionCreate) SetUserSession(u *User) *SessionCreate {
	return sc.SetUserSessionID(u.ID)
}
...
func (sc *SessionCreate) defaults() {
	...
	if _, ok := sc.mutation.MfaCompleted(); !ok {
		v := session.DefaultMfaCompleted
		sc.mutation.SetMfaCompleted(v)
	}
...
func (sc *SessionCreate) check() error {
	...
	if _, ok := sc.mutation.MfaCompleted(); !ok {
		return &ValidationError{Name: "mfa_completed", err: errors.New(`ent: missing required field "Session.mfa_completed"`)}
	}
	return nil
}
...
func (sc *SessionCreate) createSpec() (*Session, *sqlgraph.CreateSpec) {
	...
	if value, ok := sc.mutation.MfaCompleted(); ok {
		_spec.SetField(session.FieldMfaCompleted, field.TypeBool, value)
		_node.MfaCompleted = value
	}
	if nodes := sc.mutation.UserSessionIDs(); len(nodes) > 0 {
		...
		_node.user_user_session = &nodes[0]
