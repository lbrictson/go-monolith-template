...
func (sq *SessionQuery) QueryUserSession() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, session.UserSessionTable, session.UserSessionColumn),
		)
...
func (sq *SessionQuery) Clone() *SessionQuery {
	...
	return &SessionQuery{
		...
		withUserSession: sq.withUserSession.Clone(),
...
func (sq *SessionQuery) WithUserSession(opts ...func(*UserQuery)) *SessionQuery {
	query := (&UserClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withUserSession = query
	return sq
}
...
func (sq *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	var (
		...
		_spec       = sq.querySpec()
		loadedTypes = [1]bool{
			sq.withUserSession != nil,
		}
	)
	if sq.withUserSession != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withUserSession; query != nil {
		if err := sq.loadUserSession(ctx, query, nodes, nil,
			func(n *Session, e *User) { n.Edges.UserSession = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SessionQuery) loadUserSession(ctx context.Context, query *UserQuery, nodes []*Session, init func(*Session), assign func(*Session, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Session)
	for i := range nodes {
		if nodes[i].user_user_session == nil {
			continue
		}
		fk := *nodes[i].user_user_session
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_user_session" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
